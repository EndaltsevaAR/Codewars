...
public class FinderBFS {

    static Queue<NodeBFS> queueForChecking; //queue for checking nodes by levels: first there are checked first element, then all his neighbours, than all neighbours of these neighbors, so on. Element is checked and  deleted from this queue
    ...
    private static List<NodeBFS> findNeighbours(NodeBFS node) {
        List<NodeBFS> nodes = new ArrayList<>();
        NodeBFS up = new NodeBFS(node.getY() - 1, node.getX()); //for possible neighbours: up, down, left and right
    ...
    private static boolean isFinish(NodeBFS neighbour) {
        return neighbour.getY() == mazeArray.length - 1 && neighbour.getX() == mazeArray[0].length - 1;
    }

    private static boolean isRoad(NodeBFS neighbour) {
