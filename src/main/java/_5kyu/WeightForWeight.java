...
/*
Description:
My friend John and I are members of the "Fat to Fit Club (FFC)". John is worried because each month a list with the weights of members is
...
public class WeightForWeight {
    ...
    public static String orderWeight(String strng) {
        Stream<String> stream = Arrays.stream(strng.split(" "));
        return stream.sorted(Comparator.comparing(WeightForWeight::getValue).
                        thenComparing(Comparator.naturalOrder())).
                collect(Collectors.joining(" "));
    }

    private static Integer getValue(String number) {
        return Arrays.stream(number.split("")).mapToInt(Integer::parseInt).reduce(0, Integer::sum);
    }
}
